...
export function LogoCloudMarquee() {
  ...
  return (
    <section className="w-full pt-2 pb-12 sm:pb-16 px-4 sm:px-6 lg:px-8 bg-slate-50/50">
      ...
      {/* Contained 5xl Container with Soft Left & Right Fade */}
      <div
      ...
      >
        ...
        <div className="relative w-full overflow-hidden group cursor-pointer">
          <div
            className="animate-marquee group-hover:[animation-play-state:paused] flex items-center gap-8 sm:gap-12 py-3"
            style={{ animationDuration: "36s" }}
          >
            {rowOne.map((item, idx) => (
              <div
                key={`${item.name}-r1-${idx}`}
                className="shrink-0 flex items-center justify-center px-1 sm:px-2"
              >
                <img
                  src={item.src}
                  alt={item.name}
                  className="h-8 sm:h-10 w-auto max-w-[150px] object-contain select-none pointer-events-none"
                />
              </div>
            ))}
          </div>
        ...
        <div className="relative w-full overflow-hidden mt-4 sm:mt-6 group cursor-pointer">
          <div
            className="animate-marquee-reverse group-hover:[animation-play-state:paused] flex items-center gap-8 sm:gap-12 py-3"
